...
final class BlockListManager: ObservableObject {
    ...
    private func applyDomains(_ domains: [String], listID: UUID) async {
        ...
        let oldIDs = ruleStore.rules.filter { $0.notes.contains(noteTag) }.map { $0.id }
        if !oldIDs.isEmpty {
            for id in oldIDs { extensionClient.remove(ruleID: id) }
            ruleStore.remove(ids: oldIDs)
        }
        ...
        extensionClient.push(rules: newRules)
