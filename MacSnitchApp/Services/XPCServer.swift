...
extension XPCServer: NSXPCListenerDelegate {
    func listener(_ listener: NSXPCListener,
        ...
        let extensionBundleID = "com.macsnitch.extension"
        
        // In a real production app, you would also check the Team ID.
        // For now, we check the bundle identifier via the audit token's signing identifier.
        // Note: For system extensions, the connection often comes from the system-launched process.
        ...
        connection.resume()
        log.info("Accepted XPC connection from extension (pid: \(connection.processIdentifier))")
        return true
    }
}
...
private final class AppXPCHandler: NSObject, MacSnitchAppXPCProtocol {
    ...
    func logConnection(entryData: Data) {
        guard let entry = try? JSONDecoder().decode(ConnectionLogEntry.self, from: entryData) else {
            return
        }
        Task { @MainActor in
            self.delegate?.didReceiveLogEntry(entry)
        }
    }
}
