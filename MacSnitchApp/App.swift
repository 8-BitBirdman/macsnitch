...
final class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Sync rules whenever connection established (first time or restart)
        extensionClient.onReconnect = { [weak self] in
            guard let self else { return }
            self.extensionClient.push(rules: self.ruleStore.rules)
        }
        extensionClient.connect()

        extensionManager.checkStatus()
        
        // Background blocklist refresh: once on launch, then every 24 hours.
        Task {
            await blockListManager.refreshAll()
        }
        Timer.publish(every: 24 * 3600, on: .main, in: .common)
            .autoconnect()
            .sink { [weak self] _ in
                Task { await self?.blockListManager.refreshAll() }
            }
